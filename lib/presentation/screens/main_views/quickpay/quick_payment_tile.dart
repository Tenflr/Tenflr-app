  ...
  Widget build(BuildContext context) {
    ...
    final bool received =
        payment.requesterId.getOrCrash() == user.id.getOrCrash();
         if(!payment.cashed && payment.requesterId.getOrCrash() == user.id.getOrCrash()){
      ...
      child:  ListTile(
            ...
            trailing: Column(
              ...
              children: [
                  ...
                  // Divider(),
                if (payment.cashed)
                  Text(payment.date.toLocal().toString(), style: TextStyle(fontSize: size.width * 0.025),)
                else
                  Text(
                   payment.requesterId.getOrCrash() == user.id.getOrCrash()? "Uncashed" : "Sent",
                    style: TextStyle(
                        color: payment.cashed
                            ? Colors.green[300]
                            : Colors.red[300]),
                  ),
